...
export async function disconnectDappConnection(notificationPage: Page) {
  // Click `Disconnect`
  await notificationPage.locator(Selectors.menuTab.menuButton).click();
  await notificationPage.locator(Selectors.menuTab.connectedSites).click();
  await notificationPage.locator(Selectors.menuTab.disconnectButton).click();
}

export async function isDappConnected(notificationPage: Page) {
  // Check if the wallet is connected
  await notificationPage.locator(Selectors.menuTab.menuButton).click();
  await notificationPage.locator(Selectors.menuTab.connectedSites).click();
  const connectedSites = await notificationPage.locator(
    Selectors.menuTab.connectedSiteName
  ).isVisible()
