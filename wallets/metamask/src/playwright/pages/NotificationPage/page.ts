...
export class NotificationPage {
  ...
  async disconnectDappConnection(extensionId: string) {
    const notificationPage = await getNotificationPageAndWaitForLoad(
      this.page.context(),
      extensionId
    );

    await disconnectDappConnection(notificationPage);
  }

  async isDappConnected(extensionId: string) {
    const notificationPage = await getNotificationPageAndWaitForLoad(
      this.page.context(),
      extensionId
    );

    return await isDappConnected(notificationPage);
  }
