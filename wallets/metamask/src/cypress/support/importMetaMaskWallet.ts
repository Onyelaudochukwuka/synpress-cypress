...
export default async function importMetaMaskWallet(
  port: number,
  ...
  let extensionPageIndex = context
    .pages()
  ...
  if (extensionPageIndex === -1) {
    ...
    await extensionPage.goto(
      `chrome-extension://${metamaskExtensionId}/home.html`
    );
    extensionPageIndex = context
      .pages()
      .findIndex((page) => page.url().includes("chrome-extension://"));
  }
  if (extensionPageIndex !== -1) {
    ...
    const metamask = getPlaywrightMetamask(
      context,
      extensionPage,
      metamaskExtensionId
    );
